...
export class ValidationEngine {
  ...
  private async validateCredentialSchema(credential: VerifiableCredential): Promise<ValidationResult> {
    ...
    try {
      ...
      const schemaResult = schemaValidator.validateCredential(credential);
      if (!schemaResult.valid) {
        errors.push(...schemaResult.errors);
      }
      ...
      if (credential.credentialSchema) {
        ...
        for (const schema of schemas) {
          ...
          if (schema.id) {
            try {
              const specificResult = schemaValidator.validateAgainstSchema(credential, schema.id);
   ...
   */
  private async validateRevocationList2020Status(status: any, index: number): Promise<void> {
   ...
   */
  private async validateStatusList2021Entry(status: any, index: number): Promise<void> {
    // Basic field validation
   ...
   */
  async validateCredentialWithSchemas(
    credential: VerifiableCredential,
    schemaIds: string[]
  ): Promise<ValidationResult> {
    const errors: string[] = [];
    ...
    for (const schemaId of schemaIds) {
      try {
        const result = schemaValidator.validateAgainstSchema(credential, schemaId);
        if (!result.valid) {
   ...
   */
  async validateWithDetailedReport(
    credential: VerifiableCredential,
    options: ValidationOptions = {}
  ): Promise<{
    valid: boolean;
